...
def train(setting, dataset, dataset_name='mnist', save_model=False, device='CPU', dtype=tf.float32):
    ...
    model_name = "vae_{}-hei-{}-alpha{:.0e}-zd{}-hd{}-mbs{}-mbn{}-h{}-l{}-reg{:.0e}-{}-lr{:.0e}".format(generator,
                                                                                              ...
                                                                                              num_lfsteps,
                                                                                              reg, bin_label, lr)
    output_dir = "model/debug_hei_vae/"
    ...
    if save_model:
        ...
        #ckpt_name = output_dir + model_name+'/'+model_name+'ckpt'
    ...
    with tf.Session() as sess:
        ...
        for i in np.arange(0, batches):
            ...
            #if i % 1000 == 999:
            if i % 10 ==9:
                log_line = 'iter: {}, loss: {}, pot: {}, recon: {}, time: {}'.format(i + 1,
            ...
            if save_model and i % checkpoint_batch == 99:
                ...
                #saver.save(sess, output_dir, global_step=global_step)
